...
hypothesisScheirerUI <- function(id) {
  ns <- NS(id)
  ...
  verticalLayout(
    fixedRow(
      column(
        ...
        fixedRow(
          column(width = 6, HTML("")),
          column(width = 3, radioButtons(ns("pwc.method"), "Método de comparação dos pares", inline=F,
                                         choices=list("Dunn's test"="dunn", "Wilcoxon's test"="wilcoxon")))
        )
      )
    ), hr(),
    df2TableUI(ns("scheirerResultTable")), br(), hr(),
...
hypothesisScheirerMD <- function(id, data, dvs, between = c(), within = c(), wid = 'UserID', dv.var = 'var') {
  moduleServer(
    id,
    function(input, output, session) {
      ns <- session$ns
      values <- reactiveValues(scheirer.test = NULL, pair.wise = NULL, srh = list(), pwc = list())
      ...
      # .. update Scheirer results 
      
      updateScheirerResult <- function() {
        ivs <- between
        ...
        if (length(dvs) > 0) {
          ...
          values$scheirer.test <- do.call(rbind, lapply(ldvs, FUN = function(dv) {
            df <- as.data.frame(values$srh[[dv]])
            cbind(var = dv, "Effect" = rownames(df), add_significance(df))
          }))
          ...
          values$pair.wise <- do.call(rbind, lapply(dvs, FUN = function(dv) {
      ...
      observeEvent(input$pwc.method, { updateScheirerResult() })
      observeEvent(input$pairwiseCompAdjustmethod, { updateScheirerResult() })
      
      # .. displays results
      
      observeEvent(values$scheirer.test, {
        cnames <- c(dv.var, "Effect", "SSn", "SSd", "F", "p", "ges", "p.signif")
        df2TableMD("scheirerResultTable", values$scheirer.test)
      })
      ...
      observeEvent(input$pwcTabsetPanel, {
        ...
        if (nrow(pair.wise) > 0) {
          cnames <- c(".y.", "group1", "group2", "n1", "n2", "statistic",
                      "estimate", "p", "p.adj", "p.adj.signif")
        ...
        if (input$isPairwisePlot) {
          ...
          do.call(tabsetPanel, c(list(id=ns("pairwisePlotTabsetPanel"), type="pills"), lapply(dvs, function(dv) {
            tabPanel(
              dv, value = dv,
              #strong(dv),
              scheirerPlots(data, dv, ivs, srh=values$srh[[dv]], pwcs=values$pwc[[dv]],
