...
factorialAnovaMD <- function(id, initData) {
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      output$anovaTabsetUI <- renderUI({
        if (values$isSetupVars) {
          tabsetPanel(
            id = ns("anovaTabset"), type = "tabs",
            tabPanel("Teste de hipóteses", value = "hypothesisTest", hypothesisAnovaUI(ns("anovaSummaryUI"))),
      ...
      observeEvent(input$between, {
        if (length(input$between) > 3) {
          updateSelectInput(session, "between", selected = input$between[1:3])
        }
      ...
      observeEvent(values$data, {
        if (nrow(values$data) > 0) {
          hypothesisAnovaMD("anovaSummaryUI", values$data, input$dvs, between = input$between, wid = input$wid)
